...
for kk in range(3):
    ...
    for fin in range(70):
        ...
        data =np.load('/Users/betyadamkova/Desktop/final/test - model8/data/' + 'data' + str(it) +'.npy') 
        data=data[0,0,:,:]   
        data=data>0
        data=img_as_float(data)                          #binární maska
        #plt.imshow(data,cmap="gray")
        
        output =np.load('/Users/betyadamkova/Desktop/final/test - model8/output/' + 'output' + str(it) +'.npy')
        output=output[0,0,:,:]
        ...
        binar_output = remove_small_holes(remove_small_objects(binar_output, 15),12)
...
ax[1].set_title('bbinární predikovaný obraz')
ax[1].axis('off')
