...
for fin in range(70):
    ...
    output=output[0,0,:,:]
    binar_output=output > -0.32                            #binární obraz dle Otsu threshold
    binary_output=remove_small_holes(remove_small_objects(binar_output, 50),50)

    h2 =0.05    
    h2_maxima = extrema.h_maxima(output, h2)
    #seed=local_maxima(distance_output, selem=None, connectivity=50, indices=False, allow_borders=False)    #malezení středu buněk
